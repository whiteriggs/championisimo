...
// football-data.org → nombres cortos en español de la app.
const NAME_MAP = {
...
const VAPID_SUBJECT = "mailto:championisimo@whiteriggs.dev";
...
async function handleNotify(request, env, ctx) {
  ...
  const notifPayload = JSON.stringify({ title, body: body ?? "", url: clickUrl ?? "/", tag: tag ?? "championisimo" });
